    ...
    <Card className={`relative transition-all duration-200 ${current ? 'ring-2 ring-primary' : ''} ${completed ? 'bg-success/5' : ''}`}>
      <CardHeader className="pb-4">
        <div className="flex items-center gap-4">
          <div className={`p-3 rounded-full ${completed ? 'bg-success text-success-foreground' : current ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}>            {completed ? <CheckCircle className="h-6 w-6" /> : icon}
          </div>
